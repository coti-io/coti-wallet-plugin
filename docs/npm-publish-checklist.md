...
Applies to the version in this repo’s `package.json` (currently **0.4.5**). Bump `package.json` and this line together before a new publish. Unreleased API on `HEAD` is listed in `CHANGELOG.md`.

## Package Decision

- Publish package: `@coti-io/coti-wallet-plugin`
- Version: read from `package.json` (currently `0.4.5`)
